...
func (d *driver) Join(nid, eid string, sboxKey string, jinfo driverapi.JoinInfo, options map[string]interface{}) error {
	...
	// create the netlink mojanet interface
	vethName, err := createMojanet(containerIfName, n.config.Parent, n.config.MojanetMode)
	if err != nil {
		return err
	}
	...
	if len(n.config.Ipv4Subnets) > 0 {
		...
		logrus.Debugf("Mojanet Endpoint Joined with IPv4_Addr: %s, Gateway: %s, Mojanet_Mode: %s, Parent: %s",
			ep.addr.IP.String(), v4gw.String(), n.config.MojanetMode, n.config.Parent)
	}
	...
	if len(n.config.Ipv6Subnets) > 0 {
		...
		logrus.Debugf("Mojanet Endpoint Joined with IPv6_Addr: %s Gateway: %s Mojanet_Mode: %s, Parent: %s",
			ep.addrv6.IP.String(), v6gw.String(), n.config.MojanetMode, n.config.Parent)
	}
	...
	if err := d.storeUpdate(ep); err != nil {
		return fmt.Errorf("failed to save mojanet endpoint %s to store: %v", ep.id[0:7], err)
	}
	return nil
}
