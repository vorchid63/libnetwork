	...
	if opt, ok := epOptions[netlabel.PortMap]; ok {
		if _, ok := opt.([]types.PortBinding); ok {
			if len(opt.([]types.PortBinding)) > 0 {
				logrus.Warnf("%s driver does not support port mappings", mojanetType)
			}
		}
	}
	...
	if opt, ok := epOptions[netlabel.ExposedPorts]; ok {
		if _, ok := opt.([]types.TransportPort); ok {
			if len(opt.([]types.TransportPort)) > 0 {
				logrus.Warnf("%s driver does not support port exposures", mojanetType)
			}
		}
	}
...
func (d *driver) DeleteEndpoint(nid, eid string) error {
	...
	if err := d.storeDelete(ep); err != nil {
		logrus.Warnf("Failed to remove mojanet endpoint %s from store: %v", ep.id[0:7], err)
	}
