...
func (d *driver) CreateNetwork(nid string, option map[string]interface{}, nInfo driverapi.NetworkInfo, ipV4Data, ipV6Data []driverapi.IPAMData) error {
	...
	if err != nil {
		return fmt.Errorf("failed to check kernel version for %s driver support: %v", mojanetType, err)
	}
	// ensure Kernel version is >= v3.9 for mojanet support
	if kv.Kernel < mojanetKernelVer || (kv.Kernel == mojanetKernelVer && kv.Major < mojanetMajorVer) {
		return fmt.Errorf("kernel version failed to meet the minimum mojanet kernel requirement of %d.%d, found %d.%d.%d",
			mojanetKernelVer, mojanetMajorVer, kv.Kernel, kv.Major, kv.Minor)
	}
	...
	config.MojanetMode = modeBridge

	// loopback is not a valid parent link
	if config.Parent == "lo" {
		return fmt.Errorf("loopback interface is not a valid %s parent link", mojanetType)
	}
...
func (d *driver) createNetwork(config *configuration) error {
	...
	if !parentExists(config.Parent) {
		...
		if config.Internal {
			...
			err := createMojanetLink(config.Parent)
			if err != nil {
				return err
			}
	...
	if ok := n.config.CreatedSlaveLink; ok {
		...
		if ok := parentExists(n.config.Parent); ok {
			...
			if n.config.Parent == getDummyName(stringid.TruncateID(nid)) {
				...
				err := delMojanetLink(n.config.Parent)
				if err != nil {
					logrus.Debugf("link %s was not deleted, continuing the delete network operation: %v",
						n.config.Parent, err)
				}
...
func (config *configuration) fromOptions(labels map[string]string) error {
	for label, value := range labels {
		...
		case parentOpt:
			...
			config.Parent = value
		case driverModeOpt:
			// parse driver option '-o mojanet_mode'
			config.MojanetMode = value
		}
	}
