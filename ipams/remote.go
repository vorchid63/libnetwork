...
func Init(cb ipamapi.Callback, l, g interface{}) error {
     	log.Errorf("VLU-Init: remote IPAM's  init ipam driver:%s", ipamapi.PluginEndpointType)
	plugins.Handle(ipamapi.PluginEndpointType, func(name string, client *plugins.Client) {
		log.Errorf("VLU-Init: init remote IPAM driver:%s add functon:%s to endpont handlers  for plugin client=%s", 
			   ipamapi.PluginEndpointType, client )
		a := newAllocator(name, client)
		if cps, err := a.(*allocator).getCapabilities(); err == nil {
		   	log.Errorf("VLU-Init: init remote IPAM driver:%s retrieved capabilites for plugin client=%s", 
			            ipamapi.PluginEndpointType, client)
		   	log.Errorf("VLU-Init: init remote IPAM driver:%s calling plugin client=%s call back to Register IAPM with capibilit", 
			            ipamapi.PluginEndpointType, client)
...
func (a *allocator) RequestPool(addressSpace, pool, subPool string, options map[string]string, v6 bool) (string, *net.IPNet, map[string]string, error) {
        log.Errorf("VLU-RequestPool: remote ")
